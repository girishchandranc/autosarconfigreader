...
containers and parameters of the 'demo' module.
This file is generated for the module 'demo' on Fri Jan  8 11:12:56 2021.
"""
...
def read(file, ignore_previous_loaded_node=False):
    ...
    :param ignore_previous_loaded_node: Set True to ignore the previously loaded demo 
                                        via 'read'/'new' method.
    :return:  The built demo structure. May return None if the given 
              file do not contain the module configuration for demo.
    ...
    if __demoNode__ is not None and ignore_previous_loaded_node is False:
        raise ModuleAlreadyLoaded('The module demo is already loaded. To ignore the previously loaded module, please set True for argument ignore_previous_loaded_node.')
    ...
    if moduleConfNode is not None:
        __demoNode__ = demo(file, rootAutosarNode, moduleConfNode)
    else:
        raise ModuleNotFoundError('The module demo not found in the input file {}.'.format(file))
...
def new(path, ignore_previous_loaded_node=False, over_write=False, add_mandatory_containers=False):
    """
    Creates a new file and return the demo..
    ...
    :return: The built demo structure.
    ...
    arpackage = AutosarNode._create_node('AR-PACKAGE', short_name='demo')
    arpackages.append(arpackage)
    
    elements = etree.SubElement(arpackage, 'ELEMENTS')
    moduleConf = AutosarNode._create_node('ECUC-MODULE-CONFIGURATION-VALUES', short_name='demo', dest='ECUC-MODULE-DEF', definition_ref='/ModuleDef/demo')
    elements.append(moduleConf)
    et = etree.ElementTree(root)

    # Now create the demo object
    __demoNode__ = demo(path, et, moduleConf)
    # Adding mandatory containers
    if add_mandatory_containers:
        __demoNode__.new_contA('contA_0', add_mandatory_containers)
        __demoNode__.new_contC('contC_0', add_mandatory_containers)
...
def reinit():
    ...
    """
    global __demoNode__
    __demoNode__ = None # Reinit the module.
...
class demo(AutosarNode):
    ...
    def new_contA(self, name, add_mandatory_elements=False):
        ...
        containers = AutosarNode._add_or_get_xml_node(self._node, 'CONTAINERS')
        xmlNode = AutosarNode._create_node('ECUC-CONTAINER-VALUE', short_name=name, dest='ECUC-PARAM-CONF-CONTAINER-DEF', definition_ref='/ModuleDef/demo')
        ...
        if add_mandatory_elements:
            self.__add_mandatory_elements_for_contA(self.__contA, add_mandatory_elements)
        self._set_model_modified(True)
        return self.__contA

    def __add_mandatory_elements_for_contA(self, newCont, add_mandatory_elements):
        newCont.new_boolParam()
        newCont.new_enumParam()
    ...
    def new_contB(self, name, add_mandatory_elements=False):
        ...
        :raise InvalidChildNodeException: If a container with same name already exists, 
                                          or if the maximum allowed instance is reached.
        """
        xmlNode = AutosarNode._create_node('ECUC-CONTAINER-VALUE', short_name=name, dest='ECUC-PARAM-CONF-CONTAINER-DEF', definition_ref='/ModuleDef/demo/contB')
        value = contB(self, xmlNode)
        self.__add_contB(value)
        
        # Add mandatory sub-containers/parameters/references
        if add_mandatory_elements:
            self.__add_mandatory_elements_for_contB(value, add_mandatory_elements)
        return value

    def __add_mandatory_elements_for_contB(self, newCont, add_mandatory_elements):
        newCont.new_subCont('subCont_0', add_mandatory_elements)


    def get_contCs(self):
        return self.__contCs

    def __add_contC(self, value):
        """
        Adds a new contC.
        :raise InvalidChildNodeException: If a container with same name already exists, 
                                          or if the maximum allowed instance is reached.
        ...
        containers = AutosarNode._add_or_get_xml_node(self._node, 'CONTAINERS')
        if len(self.__contCs) == 5:
            raise InvalidChildNodeException('Operation not possible. According to the definition file, only a maximum of 5 instances of contC are allowed.')

        self.__contCs.append(value)
        containers.append(value._node)
        self._set_model_modified(True)

    def remove_contC(self, value):
        if value is not None:
            containers = AutosarNode._add_or_get_xml_node(self._node, 'CONTAINERS')
            containers.remove(value._node)
            self.__contCs.remove(value)
            self._set_model_modified(True)

    def removeAll_contC(self):
        for cont in self.__contCs:
            self.remove_contC(cont)


    def new_contC(self, name, add_mandatory_elements=False):
        """
        Creates a new contC with the given name.
        :param: The name of the container
        :param add_mandatory_elements: Pass True if the the mandatory sub-containers/parameters/references 
                                       for should be created(please note that this depends on the attribute 
                                       "LOWER-MULTIPLICITY" of the respective containers/parameters/references 
        ...
        :raise InvalidChildNodeException: If a container with same name already exists, 
                                          or if the maximum allowed instance is reached.
        """
        xmlNode = AutosarNode._create_node('ECUC-CONTAINER-VALUE', short_name=name, dest='ECUC-PARAM-CONF-CONTAINER-DEF', definition_ref='/ModuleDef/demo/contC')
        value = contC(self, xmlNode)
        self.__add_contC(value)
        return value
...
class contA(AutosarNode):
    ...
    def new_boolParam(self):
        ...
        xmlNode = AutosarNode._create_node('ECUC-NUMERICAL-PARAM-VALUE', dest='ECUC-BOOLEAN-PARAM-DEF', definition_ref='/ModuleDef/demo/contA/boolParam')
    ...
    def new_enumParam(self):
        ...
        paremeters = AutosarNode._add_or_get_xml_node(self._node, 'PARAMETER-VALUES')
        xmlNode = AutosarNode._create_node('ECUC-TEXTUAL-PARAM-VALUE', dest='ECUC-ENUMERATION-PARAM-DEF', definition_ref='/ModuleDef/demo/contA/enumParam')
        paremeters.append(xmlNode)
        value = AutosarNode._add_or_get_xml_node(self._node, 'VALUE')
        # set default value if available
...
class contB(AutosarNode):
    ...
    def new_subCont(self, name, add_mandatory_elements=False):
        ...
        :raise InvalidChildNodeException: If a container with same name already exists, 
                                          or if the maximum allowed instance is reached.
        """
        xmlNode = AutosarNode._create_node('ECUC-CONTAINER-VALUE', short_name=name, dest='ECUC-PARAM-CONF-CONTAINER-DEF', definition_ref='/ModuleDef/demo/contB/subCont')
        value = subCont(self, xmlNode)
        self.__add_subCont(value)
        
        # Add mandatory sub-containers/parameters/references
        if add_mandatory_elements:
            self.__add_mandatory_elements_for_subCont(value, add_mandatory_elements)
        return value
...
class subCont(AutosarNode):
    ...
    def new_intParam(self):
        ...
        paremeters = AutosarNode._add_or_get_xml_node(self._node, 'PARAMETER-VALUES')
        xmlNode = AutosarNode._create_node('ECUC-NUMERICAL-PARAM-VALUE', dest='ECUC-INTEGER-PARAM-DEF', definition_ref='/ModuleDef/demo/contB/subCont/intParam')
    ...
    def new_ref1(self):
        ...
        references = AutosarNode._add_or_get_xml_node(self._node, 'REFERENCE-VALUES')
        xmlNode = AutosarNode._create_node('ECUC-REFERENCE-VALUE', dest='ECUC-REFERENCE-DEF', definition_ref='/ModuleDef/demo/contB/subCont/ref1')
    ...
    def __add_ref2(self, value):
        """
        Adds a new ref2.
        :raise InvalidChildNodeException: If the maximum allowed instance is reached.
        """
        references = AutosarNode._add_or_get_xml_node(self._node, 'REFERENCE-VALUES')
        self.__ref2_values.append(value)
        references.append(value._node)
        self._set_model_modified(True)

    def remove_ref2(self, value):
        if value is not None:
            references = AutosarNode._add_or_get_xml_node(self._node, 'REFERENCE-VALUES')
            references.remove(value._node)
            self.__ref2_values.remove(value)
            self._set_model_modified(True)

    def removeAll_ref2(self):
        for cont in self.__ref2_values:
            self.remove_ref2(cont)
    ...
    def new_ref2(self):
        ...
        :raise InvalidChildNodeException: If the reference already exists for 
                                          single reference value.
        """
        xmlNode = AutosarNode._create_node('ECUC-REFERENCE-VALUE', dest='ECUC-REFERENCE-DEF', definition_ref='/ModuleDef/demo/contB/subCont/ref2')
        value = self.ref2(self, xmlNode)
        self.__add_ref2(value)
        return value
        if self.__ref2_value is not None:
            raise InvalidChildNodeException('Operation not possible. ref2 already exists. Use get_ref2 to access the reference.')

        references = AutosarNode._add_or_get_xml_node(self._node, 'REFERENCE-VALUES')
        xmlNode = AutosarNode._create_node('ECUC-REFERENCE-VALUE', dest='ECUC-REFERENCE-DEF', definition_ref='/ModuleDef/demo/contB/subCont/ref2')
        references.append(xmlNode)
        self.__ref2_value = self.ref2(self, xmlNode)
        self._set_model_modified(True)
        return self.__ref2_value
    ...
    def new_foreignRef(self):
        ...
        references = AutosarNode._add_or_get_xml_node(self._node, 'REFERENCE-VALUES')
        xmlNode = AutosarNode._create_node('ECUC-REFERENCE-VALUE', dest='ECUC-FOREIGN-REFERENCE-DEF', definition_ref='/ModuleDef/demo/contB/subCont/foreignRef')
        references.append(xmlNode)
